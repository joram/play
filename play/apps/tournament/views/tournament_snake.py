...
def compete(request, tournament_id):
    ...
    if request.method == "POST":
        ...
        form = TournamentSnakeForm(user=request.user, tournament=tournament, snake=snake, bracket=bracket)
    ...
    form = TournamentSnakeForm(user=request.user, tournament=tournament, snake=None, bracket=None)

    return render(
        request,
        "tournament_snake/new.html",
        {
            'form': form,
        },
    )
...
def edit(request, tournament_snake_id):
    ...
    if request.method == "POST":
        snake = Snake.objects.get(id=request.POST.get("snake"))
        form = TournamentSnakeForm(user=request.user, tournament=ts.tournament, snake=snake, bracket=ts.bracket)
        form.bracket = TournamentBracket.objects.get(id=request.POST.get("bracket"))
        if form.is_valid():
            old_ts = ts
            ts = form.save()
            if ts.snake != old_ts.snake:
                messages.success(request, f"switched snake from  {old_ts.snake.name} to {ts.snake.name}")
