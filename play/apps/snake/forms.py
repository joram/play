...
class SnakeForm(forms.ModelForm):
    name = forms.CharField(
        max_length=100, label=f"Name", required=True, widget=forms.TextInput()
    )

    url = forms.CharField(
        max_length=100, label=f"URL", required=True, widget=forms.URLInput()
    )
    ...
    def save(self, *args, **kwargs):
        ...
        UserSnake.objects.get_or_create(user=self.user, snake=snake)
        return snake
